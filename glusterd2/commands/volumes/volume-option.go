...
func xlatorAction(c transaction.TxnCtx, txnOp txnOpType, volOp xlator.VolumeOpType) error {
	...
	case xlator.VolumeStop:
		//start the daemon, if daemon option default value is "on",
		//or if particular daemon option is set to "on" in volume option
		//stop daemon, if there are no started volumes to manage
